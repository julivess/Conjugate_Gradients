...
def U__(i, j):
    return U_xy(a + j * h2, c + i * k2)
def delta_u (x, y):
    return (-2) * (y * y * (4 * x * x + y * y) * np.cos(2 * x * y * y) + x * np.sin(2 * x * y * y))
def _delta_u (j, i):
...
def Conjugate_gradient( N, M, N_max, EPS):
    global S, eps_max, n, m, h2, k2, h, k, a2, V, U, F, Nmax, eps, eps_tesk, max_r, max_ro, xm, ym
    ...
    for i in range(1, m):
        for j in range(1, n):
            if (np.abs(U[i][j] - V[i][j]) > eps_tesk):
                eps_tesk = np.abs(U[i][j] - V[i][j])
    r = Discrepancy(Matrix_multiplication(V), F)
    max_r = 0
    xm = 0
    ym = 0
